\section{Front-end}
	...
	\subsection{ReactJS}
		...
		\subsection{Quản lý router với React Router}
		
		Như đã nói, trong luận văn lần này, nhóm sẽ xây dựng hệ thống vận chuyển hàng hóa liên tỉnh theo
		mô hình Single Page Application. Nhưng để đạt được điều đó với việc dùng React
		làm framework phát triển, thì nhóm phải cần một công cụ vừa tích hợp được với React,
		vừa tận dụng và thao tác được HTML5 Routing - yếu tố chính để làm một trang web mang
		tính "SPAable". Và công cụ thích hợp nhất không ai khác chính là React Router.
		
		
		React Router và định tuyến động, phía máy khách, cho phép nhóm xây dựng Single Page App với điều hướng mà không cần làm mới trang khi người dùng điều hướng. React Router sử dụng cấu trúc thành phần để gọi các thành phần, hiển thị thông tin phù hợp.
		
		Bằng cách ngăn chặn làm mới trang và sử dụng Router hoặc Link, việc xuất hiện màn hình trắng hoặc trang trống sẽ được ngăn chặn. Đây là một cách ngày càng phổ biến để có trải nghiệm người dùng liền mạch hơn. React router cũng cho phép người dùng sử dụng chức năng trình duyệt như nút quay lại và trang làm mới trong khi duy trì giao diện chính xác của ứng dụng.
		
		\subsection{Quản lý state với Redux}
		
		Thông thường ứng dụng React sẽ quản lý các state trong mỗi components, để chia sẽ state thì phải truyền từ component cha xuống component con hoặc cần sử dụng đến context vì vậy nó chỉ phù hợp cho các ứng dụng nhỏ, có ít state. Nhưng khi ứng dụng lớn lên và có nhiều state thì việc sử dụng context sẽ khá vất vả, rất khó phát triển và bảo trì. Redux ra đời giúp chúng ta tạo ra một STORE để lưu toàn bộ data vào 1 nơi và cung cấp cho toàn bộ ứng dụng.
		
		\newpage
		
		\begin{figure}[!ht]
			\includegraphics[width=0.8\textwidth]{Images/redux.jpeg}
			\centering
			\linebreak
			\caption{Redux}
		\end{figure}
	...
	\begin{figure}[!ht]
		\includegraphics[width=0.8\textwidth]{Images/flowredux.png}
		\centering
		\linebreak
		\caption{Flow redux}
	\end{figure}
		...
		\begin{itemize}
			\item \textbf{Store}: Nơi lưu trữ tất cả state của ứng dụng. Ở đây ta có thể get, update, delete thông qua Action.
			\item \textbf{Action}: Là các hành động, event mà chúng ta tạo ra để thay đối state.
			\item \textbf{Reducer}: là 1 function nhận đầu vào là state và các mô tả về event để trả về state tiếp theo.
		\end{itemize}
	
	
		
		Flow hoạt động của Redux như sau: 
		
		\begin{itemize}
			\item Bước 1: Trên Component của người dùng xảy ra các sự kiện như người dùng click vào 1 element, tạo, update, xóa data.
			\item Bước 2: Action bắt đươc sự kiện trên component của người dùng, thực hiện tạo một hành động trong đó có type và payload.
			\item Bước 3: Action sẽ được gửi đến Reducer. Reducer sẽ dựa vào type của Action để biết và lấy data ở trong State rồi thực hiện update data đó.
			\item Bước 4: Sau khi state được thay đổi thì component chứa state đó sẽ được render lại.
		\end{itemize}
	
		Hệ thống vận chuyển hàng hóa liên tỉnh mà nhóm xây dựng gồm rất nhiều tính năng và giao diện khác nhau. Vì thế nếu không sử dụng Redux sẽ dẫn đến việc khó quản lý các state sau này. Đến một lúc nào đó thì việc mở rộng sẽ trở nên khó khăn và có thể phải đập đi xây lại từ đầu nên nhóm quyết định sử dụng Redux để quản lý state của các component trong hệ thống.
